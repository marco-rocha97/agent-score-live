...
from pydantic_ai.models import ModelProvider
...
def _obter_modelo() -> ModelProvider:
    ...
    if provider == "gemini":
        ...
        from pydantic_ai.models.gemini import GeminiModelProvider

        return GeminiModelProvider(api_key=configuracoes.gemini_api_key)
    elif provider == "openrouter":
        if not configuracoes.openrouter_api_key:
            raise ValueError("OPENROUTER_API_KEY não configurada em .env")
        from pydantic_ai.models.openai import OpenAIModelProvider
    ...
    for idx, (tipo_agente, canal, intencao, segmento_cliente, degradada) in enumerate(
        ...
        try:
            ...
            roteiro = RoteiroConversa(
                turnos=[
                    {"indice": 0, "autor": "cliente", "texto": f"Preciso de {intencao}"},
                    {"indice": 1, "autor": "agente", "texto": "Como posso ajudar?"},
                    {"indice": 2, "autor": "cliente", "texto": "Meu CPF é xxx.xxx.xxx-xx"},
                    {"indice": 3, "autor": "agente", "texto": "Vou resolver para você."},
                ]
            )
